function [ gradientParams, SigmaInvs, CholDecomps, Sigmas, bs, lastLayerResponse] = gradientCCNF( params, num_alpha, numBeta, sizeTheta, lambda_a, lambda_b, lambda_th, Precalc_Bs, x, y, Precalc_yBys, Precalc_Bs_flat, constant, num_seqs,networkConfig,mask)
...
%   Detailed explanation goes here
    ...
    vectorTheta=params(num_alpha+numBeta+1:end);
    
    thetasInit=thetaInCell(vectorTheta,networkConfig);

    
    
    % Compute the response from the neural layers in each hidden layer+X
    % itself
    
    
    
    [postAc,lastLayerResponse]=feedForward(thetasInit,x,mask);
    numLayers=size(thetasInit,2);
    
    Xt = x;
            
    bs = 2*alphas_init' * lastLayerResponse;
    ...
    if(constant)       
        ...
        %calculations for Jacobian matrix starts here
        %dynamic programming section for calculating Jacobian based on derivitive of loss function with respect to post
        %activations. Starting from last layer
        
		postAcDerivitives={};
		thetasDerivitives={};
		
		%we want all the post-activations precalculated for speed
		for i=1:numLayers
			postAcDerivitives{i}=zeros(size(thetasInit{i},1),1);
			thetasDerivitives{i}=zeros(size(thetasInit{i}));
		end
		
		
		%now we calculate all the derivitives with respect to all the weights.
		%derivitives with respect to post-activations is just utility
		%------------ this may be only ones!
		postAcDerivitives{numLayers}=bsxfun(@times, 2*repmat(alphas_init,1,size(x,2)), diff(:)');
		for i=numLayers:-1:1
			%the first thing is to calculate the derivitive of the activation
			%function. 
			
			%derivative of post activation with respect to pre-activations *
			%derivative of error with respect to post-activation. 
            bias=repmat([1],1,size(x,2));
			dOfE_PreAc_i=(postAc{i}.*(1-postAc{i}));
            dOfE_PreAc_i=dOfE_PreAc_i.*postAcDerivitives{i};
			dOfE_PreAc_i=dOfE_PreAc_i.*repmat(mask{i}',1,size(x,2));
			%we calculate derivitives with respect to post-activations
			if(i==1)
				thetasDerivitives{i}=(dOfE_PreAc_i)*x';
            else
				thetasDerivitives{i}=(dOfE_PreAc_i)*cat(1,postAc{i-1},bias)';
				postAcDerivitives{i-1}=thetasInit{i}(:,1:end-1)'*(dOfE_PreAc_i);
			end
		end
        
        
        gradientThetasT=thetaInVector(thetasDerivitives);
        ...
        gradient_alphas = 2 * lastLayerResponse * (y(:) - mus(:)) + gradient_alphas_add;
...
%}      
    ...
    regTheta = vectorTheta * lambda_th;
    
    
    gradientParams = gradientParams - 2*[regAlpha; regBeta; regTheta(:)];
end
