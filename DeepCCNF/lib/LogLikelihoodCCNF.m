...
if(~const)
    ...
    if(isempty(SigmaInvs))
        ...
        [postAc,all_X_resp]=feedForward(thetasInit,xs);
...
thetasCopy=thetaInVector(thetasInit);
% add regularisation term
logL = logL -lambda_b * (betas'*betas)/2 - lambda_a * (alphas'*alphas)/2 - lambda_th * (thetasCopy(:)'*thetasCopy(:))/2;
