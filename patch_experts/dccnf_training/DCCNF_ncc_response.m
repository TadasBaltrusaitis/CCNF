function [ responses ] = DCCNF_ncc_response( patches, patch_experts, normalisation_options, window_size, patch_length)
...
%   Using convolution, for testing purposes and not for actual speed
    ...
    for i = 1:size(patches{1},1)
        ...
        hl_per_modality = size(patch_experts.thetas{1},1);
        final_layer_num = size(patch_experts.thetas{end},1);
        responses_conv = zeros(hl_per_modality, patch_length);
        ...
        for p=1:num_modalities
            ...
            for hls = 1:hl_per_modality
                ...
                w = patch_experts.thetas{1}(hls, 2:end, p);
                norm_w = norm(w);
                w = w/norm(w);
                w = reshape(w, patchSize);

                response = -norm_w * Cross_corr_resp(smallRegion, w, norm_cross_corr, patchSize) - patch_experts.thetas{1}(hls,1,p);
                response = 1./(1 + exp(response(:)));
                responses_conv(hls,:) = response;
            end
