...
tic
% Do it across all of the test folds
for test_fold = 1:num_test_folds
    ...
    %% Keep redoing because of local optimum thing
    for i=1:numRedo
...
%     [alphasCCNF, betasCCNF, thetasCCNF, ~] = CCNF_training_bfgs(thresholdX, thresholdFn, trainSamples{test_fold}, trainLabels_val_normed{test_fold}, alphas, betas, initial_thetas_val{ind}, best_lambda_a_val, best_lambda_b_val, best_lambda_th_val, similarities, 'const', true);   
    % [corrsTrain, rmssTrain, meanCorrTrain, meanRMSTrain, longCorrTrain, longRMSTrain ] = evaluateCCNFmodel(alphasCCNF, betasCCNF, thetasCCNF, trainSamples, trainLabels_val, similarities, offset_val, scaling_val, true);
...
%%
save('results/CCNF', 'best_lambdas_a_ar', 'best_lambdas_b_ar', 'best_lambdas_th_ar', 'best_num_layers_ar', ...
