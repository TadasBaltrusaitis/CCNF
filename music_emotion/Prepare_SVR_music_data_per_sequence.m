function [trainSamples, testSamples, trainLabels_ar, trainLabels_ar_normed, testLabels_ar, trainLabels_val, trainLabels_val_normed, testLabels_val, offsets_val, offsets_ar, scalings_val, scalings_ar, num_features] = Prepare_SVR_music_data_per_sequence(num_folds_test, add_bias)
    ...
    %%
    sampleLength = 15;
    num_features = sampleLength * size(instance_matrix_ar,2);
    ...
    for f=1:num_folds_test
        ...
        size_train_fold = sum(inds_train_fold);
        size_test_fold = sum(inds_test_fold);
        ...
        for i=1:numSamples
            ...
            sample = sample';
            sample = sample(:);
            labelAr = mean(label_vector_ar((i-1)*sampleLength+1:(i)*sampleLength));
            labelVal =  mean(label_vector_val((i-1)*sampleLength+1:(i)*sampleLength));
            if(inds_train_fold(i))
                trainSamples_fold(countTrain,:) = sample;
                trainLabels_ar_fold(countTrain) = labelAr;
                trainLabels_val_fold(countTrain) = labelVal;
                countTrain = countTrain + 1;        
            else
                testSamples_fold(countTest,:) = sample;
                testLabels_ar_fold(countTest) = labelAr;
                testLabels_val_fold(countTest) = labelVal;
