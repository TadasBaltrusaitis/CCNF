...
for test_fold = 1:num_test_folds
    ...
    [ best_params_ar, all_params_ar ] = crossvalidate_grid_search(train, test, true, trainSamples{test_fold}, trainLabels_ar{test_fold}, hyperparams, 'inds_cv_train', inds_cv_train, 'num_repeat', 2);
    
    hyperparams.centres = centres_val(test_fold,:);
    
    % Crossvalidate valence model
    [ best_params_val, all_params_val ] = crossvalidate_grid_search(train, test, true, trainSamples{test_fold}, trainLabels_ar{test_fold}, hyperparams, 'inds_cv_train', inds_cv_train, 'num_repeat', 2);
